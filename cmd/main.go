...
import (
	...
	"github.com/lonegunmanb/terraform-azapi-schema/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh azapi schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-azapi-schema", options)
		if err != nil {
			return false, err
		}
